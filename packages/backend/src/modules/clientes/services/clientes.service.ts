...
export class ClientesService {
  ...
  async listar(filtros: ConsultarClienteDto) {
    ...
    const [clientes, total] = await this.prisma.$transaction([
      this.prisma.cliente.findMany({
        ...
        include: { enderecos: { where: { isPrincipal: true }, take: 1 }, contatos: true },
      }),
  ...
  async adicionarEndereco(clienteId: string, dto: CreateEnderecoDto): Promise<ClienteResponseDto> {
    ...
    const response = await this.prisma.$transaction(async (tx) => {
      ...
      if (isPrincipal) await this.desmarcarEnderecosPrincipais(tx, clienteId);
  ...
  async atualizarEndereco(
    ...
    const response = await this.prisma.$transaction(async (tx) => {
      ...
      if (dto.isPrincipal === true) await this.desmarcarEnderecosPrincipais(tx, clienteId);
      ...
      await this.auditoriaService.registrarOperacao(tenantId, usuario, {
        ...
        dadosAnteriores: enderecoAnterior,
        dadosAtuais: endereco,
      });
  ...
  async atualizarContato(
    ...
    const response = await this.prisma.$transaction(async (tx) => {
      ...
      await this.auditoriaService.registrarOperacao(tenantId, usuario, {
        ...
        recurso: 'cliente_contato',
        recursoId: contatoId,
        dadosAnteriores: contatoAnterior,
        dadosAtuais: contato,
      });
  ...
  private async buscarEnderecoDoCliente(
    ...
    if (!endereco) throw new NotFoundException(`Endereco "${enderecoId}" nao encontrado para o cliente.`);
    return endereco;
  }
  ...
  private async buscarContatoDoCliente(
    ...
    if (!contato) throw new NotFoundException(`Contato "${contatoId}" nao encontrado para o cliente.`);
    return contato;
  }
  ...
  private limparDocumentoObrigatorio(documento: string): string {
    ...
    if (!documentoLimpo) throw new BadRequestException('Documento e obrigatorio.');
    return documentoLimpo;
  }

  private limparCep(cep: string): string {
    const cepLimpo = cep.replace(/[^\d]/g, '');
    if (cepLimpo.length !== 8) throw new BadRequestException('CEP deve conter 8 digitos.');
    return cepLimpo;
  }
