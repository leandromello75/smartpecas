...
let AuditoriaService = AuditoriaService_1 = class AuditoriaService {
    ...
    async registrarOperacao(tenantId, usuario, dados) {
        ...
        this.logger.log(`[AUDITORIA - ${tenantId}] ${operacao} em ${recurso}:${recursoId} por ${usuario.name ?? usuario.email} (ID: ${usuario.sub})`);
        setImmediate(async () => {
            try {
                await this.prisma.auditoriaLog.create({
                    data: {
                        ...
                        realizadoPorNome: usuario.name ?? usuario.email ?? null,
                        realizadoPorIp: ip ?? usuario.ip ?? null,
                        userAgent: userAgent ?? usuario.userAgent ?? null,
                    },
                });
